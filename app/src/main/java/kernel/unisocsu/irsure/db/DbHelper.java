...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public boolean hasData() {
        ...
        Cursor c = db.rawQuery("SELECT 1 FROM codesets LIMIT 1", null);
        try {
    ...
    public AcFunction findFunction(long codesetId, String power, String mode, Integer temp, String fan, String swing) {
        SQLiteDatabase db = getReadableDatabase();
        String sql = "SELECT * FROM functions WHERE codeset_id=? AND power=? AND (mode=? OR mode IS NULL) AND (temp=? OR temp IS NULL) AND (fan=? OR fan IS NULL) AND (swing=? OR swing IS NULL)";
        Cursor c = db.rawQuery(sql, new String[]{String.valueOf(codesetId), power, mode, String.valueOf(temp), fan, swing});
        if (c.moveToFirst()) {
    ...
    public long insertScheduledTask(long codesetId, long timeMillis, int actionType, String power, String mode, Integer temp, String fan, String swing) {
        ...
        return db.insert("scheduled_tasks", null, values);
    }
}
